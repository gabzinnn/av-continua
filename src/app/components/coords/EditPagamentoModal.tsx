...
import { updatePagamento, UpdatePagamentoInput, PagamentoCompleto } from "@/src/actions/pagamentosActions"
...
import { StatusPagamento } from "@/src/generated/prisma/client"
...
export function EditPagamentoModal({ isOpen, onClose, onSuccess, areas, demandas, pagamento }: EditPagamentoModalProps) {
    ...
    const [formData, setFormData] = useState<Omit<UpdatePagamentoInput, "id">>({
        nome: "",
        descricao: "",
        valor: 0,
        notaFiscal: "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await updatePagamento({
                id: pagamento.id,
                ...formData,
                pdfUrl,
            })
            ...
            if (result.success) {
                ...
                setError(result.error || "Erro ao atualizar gasto")
            }
        } catch {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-bg-card rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between p-6 border-b border-border">
                    <h2 className="text-xl font-bold text-text-main">Editar Gasto</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setFormData({ ...formData, status: e.target.value as StatusPagamento })}
                            className="w-full px-4 py-2.5 border border-border rounded-lg bg-bg-main focus:outline-none focus:border-primary cursor-pointer"
                        >
                    ...
                    {/* Upload PDF */}
                    <div>
                        <label className="block text-sm font-medium text-text-main mb-1">Nota Fiscal (PDF)</label>
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-medium text-gray-700">
                                            <span className="text-primary hover:underline">
                                                {formData.pdfUrl ? "Substituir PDF" : "Clique para enviar"}
                                            </span>
                                        </p>
                                        <p className="text-xs text-gray-500">
                                            Apenas PDF (máx. 10MB)
