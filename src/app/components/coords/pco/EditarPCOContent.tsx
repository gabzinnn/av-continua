...
import { Plus, Trash2, X, ChevronDown, ChevronUp, ArrowLeft, FolderPlus } from "lucide-react"
import { Button } from "@/src/app/components/Button"
import { editarPCO, CriarPCOInput, PCOParaEditar } from "@/src/actions/pcoActions"
...
export function EditarPCOContent({ pcoInicial }: { pcoInicial: PCOParaEditar }) {
    const router = useRouter()

    const [nome, setNome] = useState(pcoInicial.nome)
    const [descricao, setDescricao] = useState(pcoInicial.descricao ?? "")
    const [secoes, setSecoes] = useState<SecaoForm[]>(
        pcoInicial.secoes.map((s) => ({
            titulo: s.titulo,
    ...
    const handleSubmit = async () => {
        ...
        const result = await editarPCO(pcoInicial.id, input)
                ...
                <div className="px-8 py-6 max-w-[960px] mx-auto w-full flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap justify-between items-end gap-4">
                        <div className="flex flex-col gap-1">
                            <h1 className="text-text-main text-3xl font-bold tracking-tight">Editar PCO</h1>
                            <p className="text-text-muted text-base">Edite o rascunho da Pesquisa de Clima Organizacional</p>
                        </div>
                        <div className="text-sm text-text-muted bg-bg-card border border-border rounded-lg px-4 py-2">
                            {perguntasTotal} pergunta{perguntasTotal !== 1 ? "s" : ""} · {secoes.length} seção{secoes.length !== 1 ? "ões" : ""}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="px-8 pt-8">
                <div className="max-w-[960px] mx-auto w-full">
                            ...
                            <div className="flex flex-col gap-5">
                                <div className="flex flex-col gap-2">
                                    ...
                                    <input
                                        ...
                                        placeholder="Ex: Pesquisa de Clima 2024.1"
                                        className="w-full rounded-lg border border-border bg-bg-main px-4 py-3 text-text-main placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary text-sm transition-all"
                                    />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-semibold text-text-main">Descrição</label>
                                    <textarea
                                        value={descricao}
                                        onChange={(e) => setDescricao(e.target.value)}
                                        placeholder="Descreva o objetivo da pesquisa (opcional)..."
                                        rows={3}
                                        className="w-full rounded-lg border border-border bg-bg-main px-4 py-3 text-text-main placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary text-sm transition-all resize-none"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    {secoes.map((secao, sIndex) => (
                        <div
                            key={sIndex}
                            className="bg-bg-card rounded-xl shadow-sm border border-border overflow-hidden mb-4"
                        >
                            {/* Section Header */}
                            <div className={`flex items-center gap-3 px-6 py-4 bg-bg-main ${secao.expanded ? "border-b border-border" : ""}`}>
                                ...
                                <div className="flex-1 flex flex-col gap-1">
                                    <input
                                        ...
                                        className="w-full bg-transparent font-bold text-text-main border-none p-0 focus:ring-0 outline-none placeholder:text-text-muted/50"
                                    />
                                    <input
                                        type="text"
